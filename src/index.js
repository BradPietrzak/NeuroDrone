...
const thoughts = ["leftHandPinch", "rightHandPinch", "tongue", "raiseEyebrows"];
...
const trainThought = () => {
  ...
  neurosity.kinesis(thoughtToTrain).subscribe((intent) => {
    if (intent.confidence > 0.8) { // Adjust confidence level as needed
      console.log(`Detected ${thoughtToTrain} with high confidence!`);
      // Add any additional actions you'd like here
    }
  });
...
const main = async () => {
  ...
  // Listen for user input to toggle thoughts
  rl.on("line", (input) => {
